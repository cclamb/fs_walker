...
%% Name must be an atom, not a string.
...
handle_msg({send_event, Event}, LoopData) ->
    {ok, handle_event(Event, LoopData) }.

handle_event(_Event, []) -> [];

handle_event(Event, [{Handler, Data} | Rest]) ->
    [{Handler, Handler:handle_event(Event, Data) } | handle_event(Event, Rest)].
