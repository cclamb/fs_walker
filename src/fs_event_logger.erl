...
handle_event({work_added, Pkg}, State) ->
    error_logger:info_message("work added(~p)~n", [Pkg]),
    {ok, State};

handle_event({orphan_died}, State) ->
    error_logger:warning_message("orphan process_died()", []),
    {ok, State};

handle_event({orphan_died, Pid}, State) ->
    error_logger:warning_message("orphan process_died(~w)", [Pid]),
    {ok, State};

handle_event({process_died, Pid}, State) ->
    error_logger:warning_message("process_died(~w)", [Pid]),
    fs_workserver:process_died(Pid),
    {ok, State};

handle_event({orphan_work, Pid}, State) ->
    error_logger:info_message("Pid ~p) reports work done, but work not assigned", [Pid]),
    {ok, State#state{file_errors = State#state.file_errors + 1}};

handle_event({file_error, FileName}, State) ->
    error_logger:info_message("file_error(~p)", [FileName]),
    {ok, State#state{file_errors = State#state.file_errors + 1}};
